...
using Kepler_Demo.Interfaces.Demo.v1.Exceptions;
using Kepler_Demo.Interfaces.Demo.v1.Models;

namespace Kepler_Demo.Services.Demo.v1
{
	public class WorkspaceName : IWorkspaceName
	{
		private IHelper _helper;
		private ILog _logger;

		// Note: IHelper and ILog are dependency injected into the constructor every time the service is called.
		public WorkspaceName(IHelper helper, ILog logger)
		{
			// Note: Set the logging context to the current class.
			_logger = logger.ForContext<WorkspaceName>();
			_helper = helper;
		}

		public async Task<WorkspaceNameModel> GetWorkspaceNameAsync(int workspaceID)
		{
			WorkspaceNameModel model;
			...
			catch (Exception exception)
			{
				...
				throw new WorkspaceNameException($"Workspace {workspaceID} not found.")
				{
					FaultSafeObject = new WorkspaceNameException.FaultSafeInfo()
					{
		...
		public async Task<List<WorkspaceNameModel>> QueryWorkspaceByNameAsync(string queryString, int limit)
		{
			var models = new List<WorkspaceNameModel>();
			...
			var proxy = _helper.GetServicesManager().CreateProxy<IWorkspaceName>(ExecutionIdentity.CurrentUser);
			...
			try
			{
				...
				foreach (int workspaceID in workspaceIDs)
				{
					...
					//       See also https://blog.stephencleary.com/2012/07/dont-block-on-async-code.html
					//       Warning: Improper use of the tasks can cause deadlocks and performance issues within an application.
					WorkspaceNameModel wsModel = await proxy.GetWorkspaceNameAsync(workspaceID).ConfigureAwait(false);
					if (wsModel != null)
					{
						models.Add(wsModel);
					}
			...
			catch (Exception exception)
			{
				...
				throw new WorkspaceNameException($"An exception occured during query for workspace(s) containing {queryString}.");
			}
